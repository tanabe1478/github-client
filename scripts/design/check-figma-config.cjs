...
if (!["unbound", "active", "archived"].includes(manifest.status)) {
  errors.push("status must be unbound, active, or archived");
}
...
if (manifest.status === "active") {
  if (!/^https:\/\/(www\.)?figma\.com\//.test(manifest.file.url)) {
    errors.push("an active binding requires a figma.com file URL");
  }
  if (!manifest.file.key) errors.push("an active binding requires file.key");
}
